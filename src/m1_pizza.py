...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(200, 150.0),  # All numbers are approximate.
                rg.Point(150, 200),
                rg.Point(100, 150),
                rg.Point(150, 100)]
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(pts)):
        newcirc = rg.Circle(pts[k], 10)
        newcirc.fill_color = color
        newcirc.attach_to(window)
        pts[k].attach_to(window)
...
def run_test_fancy_polygon():
    ...
    title = ('FANCY POLYGON test 4:  300 purple lines on blue circle,'
             + ' hops = 125.')
    window = rg.RoseWindow(500, 500, title)

    circle = rg.Circle(rg.Point(250, 250), 150)
    circle.fill_color = 'orange'
    fancy_polygon(window, circle, 150, 60, 'black', 2)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(len(pts)):
        line = rg.Line(pts[k], pts[(k + hops_to_next_point) % number_of_lines])
        line.color = color
        line.thickness = thickness
        line.arrow = 'last'
        line.attach_to(window)
